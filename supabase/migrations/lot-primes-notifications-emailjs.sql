-- ============================================================
-- AeroPrimes — notifications email GRATUITES via EmailJS + Gmail
-- À exécuter UNE SEULE FOIS dans Supabase > SQL Editor
-- (remplace l'ancien envoi Brevo, aucun domaine nécessaire)
--
-- Prérequis (gratuits) :
--   1. Un compte Gmail dédié + « mot de passe d'application »
--      (Google > Compte > Sécurité > validation en 2 étapes
--       > Mots de passe d'application)
--   2. Un compte sur https://emailjs.com :
--      - service Gmail (identifiant = l'adresse Gmail,
--        mot de passe = le mot de passe d'application)
--      - un modèle (template) : À = {{to_email}},
--        Sujet = {{subject}}, Contenu = {{{message}}}
--      - récupérer : Service ID, Template ID, clé publique
--        (user_id) et clé privée (secret)
-- ============================================================

-- 1) Colonnes EmailJS dans la configuration
alter table public.notify_config
  add column if not exists emailjs_service_id text not null default '';

alter table public.notify_config
  add column if not exists emailjs_template_id text not null default '';

alter table public.notify_config
  add column if not exists emailjs_public_key text not null default '';

alter table public.notify_config
  add column if not exists emailjs_private_key text not null default '';

-- 2) Envoi d'un email via l'API EmailJS (gratuite, sans domaine)
